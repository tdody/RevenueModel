...
def _model_train(df,tag,test=False):
    ...
    ## update log
    update_train_log(tag,str(dates[0]),str(dates[-1]),X.shape,{'rmse':eval_rmse},runtime,MODEL_VERSION, MODEL_VERSION_NOTE,test=True)
...
def model_load(prefix='sl',data_dir=None,training=True, country=None):
    ...
    if country is None: 
        ...
        models = [f for f in os.listdir(os.path.join(".","models")) if (re.search(prefix,f) and re.search(country_id,f))]

    if len(models) == 0:
        if country is None:
            raise Exception("Models with prefix '{}' cannot be found, did you train?".format(prefix))
        else:
            raise Exception("Model for '{0}' with predix '{1}' cannot be found, did you train it?".format(prefix, country))
...
def model_predict(country,year,month,day,all_models=None, all_data=None,test=False):
    ...
    if not all_models:
        if test:
            all_data, all_models = model_load(training=False, prefix="test", country=country)
        else:
            print("... loading model")
            all_data, all_models = model_load(training=False, prefix="sl", country=country)
    ...
    if target_date not in data['dates']:
        raise Exception("ERROR (model_predict) - date {} not in range {} to {}".format(target_date,
                                                                                    data['dates'][0],
                                                                                    data['dates'][-1]))
    ...
    ## update predict log
    update_predict_log(y_pred,country+"_"+target_date,runtime,MODEL_VERSION,test=test)
...
if __name__ == "__main__":
    ...
    ## locate data
    data_dir = os.path.join(".","data","cs-train")
    ## train all models (load, transform, fit)
    model_train(data_dir,test=False)

    ## load the model
    print("LOADING MODELS")
    all_data, all_models = model_load(prefix='sl')
